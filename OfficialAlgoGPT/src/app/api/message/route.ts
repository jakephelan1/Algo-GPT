...
//     const slideContext = body.slideContext ? `The user is asking about this slide:\n${body.slideContext}\n\n` : "";


//     const systemMessage = slideContext + (await ChatbotPrompt(body.id, body.solution, body.desc));
...
export async function POST(req: Request) {
    ...
    const slideContextMessage: ChatGPTMessage | null = body.slideContext
        ? {
            role: "system" as const, // 
            content: `The user is asking about this slide:\n${body.slideContext}\n\n`
        }
        : null;

    const systemMessage: ChatGPTMessage = {
        role: "system" as const, 
        content: await ChatbotPrompt(body.id, body.solution, body.desc),
    };

    if (slideContextMessage) {
        outboundMessages.unshift(slideContextMessage);
    }

    outboundMessages.unshift(systemMessage);
